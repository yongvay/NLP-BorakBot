...
""")
    ...
    D.table(at, ["Path", "Purpose"],
            [["app/streamlit_app.py", "Chat interface, voice and text"],
             ...
             ["app/pages/admin.py", "Password-gated feedback review"],
             ["stage1_stt/run_wer.py", "The WER sweep: 9 decode configurations over 48 clips"],
             ["stage1_stt/threshold_sweep.py", "Justifies the English routing threshold"],
             ["stage1_stt/manifest.csv", "The 48-sentence stratified speech test set"],
             ["stage1_stt/results/", "Committed result files behind every speech figure"],
             ["stage2_chatbot/knowledge_base/", "Five domain YAML files, scope rules, system prompt"],
             ["stage2_chatbot/corpus/", "Corpus and the train/validation/test splits"],
             ["stage2_chatbot/corpus_tooling/", "Corpus construction and the generation prompt (Appendix A)"],
             ["stage2_chatbot/schema.sql", "Feedback table definition (the database is not committed)"],
             ["stage2_chatbot/training/qlora_config.yaml", "Fine-tuning configuration"],
             ["stage2_chatbot/training/to_llamafactory.py", "Corpus to training format, stamping the prompt"],
             ["stage2_chatbot/training/colab_finetune.ipynb", "The training run"],
             ["stage2_chatbot/training/colab_evaluate.ipynb", "The graded comparison of section 14"],
             ["stage2_chatbot/eval/generate.py", "Runs a model over a split and records what it said"],
             ["stage2_chatbot/eval/score.py", "Perplexity, BLEU, chrF, ROUGE-L, BERTScore"],
             ["stage2_chatbot/eval/refusal_report.py", "Fallback accuracy and over-refusal"],
             ["stage2_chatbot/eval/results/", "Committed result files behind every figure here"]])
    D.caption(at, "Repository layout. The repository is organised around the two stages the "
                  "system is built from -- speech to text, then the chatbot -- with the live "
                  "application in app/ drawing on both. The live application is five modules "
                  "plus the interface; everything else is data, evaluation or documentation.")
...
def appendices(D, e):
    ...
    D.para_end("""Read from stage2_chatbot/knowledge_base/_system_prompt.md by the training converter, the evaluation harness and the application, so that the text cannot drift between training, measurement and demonstration.""")
