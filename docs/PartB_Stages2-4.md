...
*Table 9: QLoRA configuration. The full file is stage2_chatbot/training/qlora_config.yaml.*
...
*Table 14: Repository layout. The repository is organised around the two stages the system is built from -- speech to text, then the chatbot -- with the live application in app/ drawing on both. The live application is five modules plus the interface; everything else is data, evaluation or documentation.*
...
Read from stage2_chatbot/knowledge_base/_system_prompt.md by the training converter, the evaluation harness and the application, so that the text cannot drift between training, measurement and demonstration.
